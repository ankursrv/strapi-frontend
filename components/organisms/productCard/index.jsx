...
const ProductCards = () => {
  ...
  // useEffect (()=>{
  //   async function getSeeMoreButton() {
  //     const res = await fetch(
          ...
          <div key={index} className="max-w-[200px]">
            ...
            <div className="flex items-center justify-between mt-2">
              ...
              <Link href={item.button?.href || "#"}>
                <button className="px-4 py-1.5 text-gray-500 border border-gray-500/20 rounded-full text-xs hover:bg-slate-50 transition">{item.button?.text}</button>
              </Link>
            </div>
          </div>
        ))}
